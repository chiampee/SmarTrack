...
import { downloadEventService } from '../services/downloadEventService';
...
export const AdminStatsPage: React.FC = () => {
  ...
  const [downloadStats, setDownloadStats] = useState<{
    ...
    recentDownloads: Array<{ userId: string; source: string; timestamp: string; userAgent: string; ip: string }>;
  ...
  const [installationStats, setInstallationStats] = useState<{
    ...
    installationsByUser: Record<string, number>;
    installationsByDay: Record<string, number>;
    recentInstallations: Array<{ userId: string; extensionVersion: string; installTimestamp: string; browserInfo: string; userAgent: string; ip?: string }>;
  } | null>(null);
  ...
  const buildStats = async (): Promise<StatItem[]> => {
    ...
    return [
      ...
      { label: 'Extension Downloads', value: downloadStats?.totalDownloads || 0 },
      { label: 'Extension Installations', value: installationStats?.totalInstallations || 0 },
    ];
  };
  ...
  const fetchDownloadStats = async () => {
    try {
      const stats = await downloadEventService.getDownloadStats();
  ...
  const fetchInstallationStats = async () => {
    try {
      const stats = await extensionInstallationService.getInstallationStats();
  ...
  const generateUsageChartData = async () => {
    try {
      const [downloadStats, installationStats, userAudits] = await Promise.all([
        downloadEventService.getDownloadStats(),
        extensionInstallationService.getInstallationStats(),
        db.userAudits.toArray()
      ]);

      const { dateRange } = chartControls;
      ...
      // Simplified daily view only for now
  ...
  useEffect(() => {
    if (downloadStats && installationStats) {
      generateUsageChartData();
    }
  }, [chartControls]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* PM Insights & Analytics */}
        {usageChartData && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
            {/* Login Analytics */}
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Login Activity</h3>
                <div className="text-right">
                  <div className="text-lg font-bold text-blue-600">
                    {usageChartData.logins.reduce((a, b) => a + b, 0)}
                  </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium">{Math.max(...usageChartData.logins)}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-gray-600">Daily Avg:</span>
                  <span className="font-medium">
                    {Math.round(usageChartData.logins.reduce((a, b) => a + b, 0) / usageChartData.logins.length)}
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-gray-600">Active Days:</span>
                  <span className="font-medium">{usageChartData.logins.filter(count => count > 0).length}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-purple-600">
                    {Math.round((usageChartData.installations.reduce((a, b) => a + b, 0) / Math.max(usageChartData.logins.reduce((a, b) => a + b, 0), 1)) * 100)}%
                  </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs">
                  <span className="text-gray-600">Logins → Downloads:</span>
                  <span className="font-medium text-green-600">
                    {Math.round((usageChartData.downloads.reduce((a, b) => a + b, 0) / Math.max(usageChartData.logins.reduce((a, b) => a + b, 0), 1)) * 100)}%
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-gray-600">Downloads → Installs:</span>
                  <span className="font-medium text-purple-600">
                    {Math.round((usageChartData.installations.reduce((a, b) => a + b, 0) / Math.max(usageChartData.downloads.reduce((a, b) => a + b, 0), 1)) * 100)}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                  <div 
                    className="bg-gradient-to-r from-green-500 to-purple-500 h-1.5 rounded-full" 
                    style={{ 
                      width: `${Math.round((usageChartData.installations.reduce((a, b) => a + b, 0) / Math.max(usageChartData.logins.reduce((a, b) => a + b, 0), 1)) * 100)}%` 
                    }}
                  ></div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-green-600">
                    {usageChartData.logins.length > 1 ? 
                      (usageChartData.logins[usageChartData.logins.length - 1] > usageChartData.logins[0] ? '↗' : '↘') : '—'
                    }
                  </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium text-green-600">
                    {usageChartData.downloads.length > 1 ? 
                      (usageChartData.downloads[usageChartData.downloads.length - 1] > usageChartData.downloads[0] ? '↗' : '↘') : '—'
                    }
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium text-purple-600">
                    {usageChartData.installations.length > 1 ? 
                      (usageChartData.installations[usageChartData.installations.length - 1] > usageChartData.installations[0] ? '↗' : '↘') : '—'
                    }
                  </span>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium text-gray-700">Links (7d)</div>
              <svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
            </div>
            <div className="text-2xl font-bold text-gray-900">{recent.links7d}</div>
            <div className="text-xs text-gray-500">New links this week</div>
          </div>
          
          <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium text-gray-700">Chats (7d)</div>
              <svg className="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
            </div>
            <div className="text-2xl font-bold text-gray-900">{recent.chats7d}</div>
            <div className="text-xs text-gray-500">AI conversations this week</div>
          </div>
          
          <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium text-gray-700">Users (7d)</div>
              <svg className="w-4 h-4 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" />
              </svg>
            </div>
            <div className="text-2xl font-bold text-gray-900">{recent.users7d}</div>
            <div className="text-xs text-gray-500">Users active this week</div>
          </div>
        </div>
